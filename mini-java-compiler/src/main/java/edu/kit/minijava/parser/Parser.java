...
public final class Parser {
    ...
    private final List<edu.kit.minijava.lexer.Token> tokens = new ArrayList<>();
    ...
    private Token consume(TokenType type) throws ParserException{
        ...
        if (token == null) {
            throw new ParserException("Expected to read " + type +  ", but found end of file");
        } else if (token.type != type) {
            throw new ParserException("Expected to read " + type +  ", but found " + token.type +
            ...
            switch (this.getCurrentToken().type) {
                case STATIC: {
                    ...
                    if (!parameterType.equals("String")) {
                        throw new ParserException("Main method does not have the right argument: "
    ...
    private BlockStatement parseBlockStatement() throws ParserException {
        if (this.hasReachedEndOfInput()) {
            throw new ParserException("Cannot parse BlockStatement because End of Input has been reached");
        }
